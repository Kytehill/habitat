...
class Ssh:
    ...
    def execute_commands(self, environment, server, commands):
        ...
        for command in commands:
            ...
            for line in output:
                ...
                self.compare(command.id, command.expectation, final_line)
    ...
    def multithread(self):
        ...
        while True:
            for environment in environments:
                servers = self.get_servers(environment.id)
                for server in servers:
                    ...
                    t = threading.Timer(int(environment.timing), self.execute_commands, args=(environment, server, commands))
                    # time.sleep(int(environment.timing))
                    t.start()
    ...
    def compare(self, command_id, expectation, live_output):
